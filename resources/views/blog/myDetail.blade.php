...
<div class="row mt-5 mb-5 col-md-8 mx-auto border border-dark rounded p-5 bg-dark">
  <div class="col-md-offset-2">
      <h2 class="text-info">{{$myBlog->title}}</h2>
      <p class="text-light mt-2">ユーザー名：{{$myBlog->getUserName()}}</p>
      <span class="text-light me-2">作成日{{$myBlog->created_at->format('Y/m/d')}}</span>
      <span class="text-light">更新日{{$myBlog->updated_at->format('Y/m/d')}}</span>
      <p class="text-info mt-3">どんな失敗をした？</p>
      <p class="text-light">{{$myBlog->content_a}}</p>
      <p class="text-info mt-3">原因は何だった？</p>
      <p class="text-light">{{$myBlog->content_b}}</p>
      <p class="text-info mt-3">みんなに同じミスをして欲しくないとしたら、どうアドバイスする？</p>
      <p class="text-light">{{$myBlog->content_c}}</p>
  </div>
</div>
@endsection
@include('footer')